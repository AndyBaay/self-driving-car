...
with graph.as_default():
    ...
    W1 = tf.Variable(tf.random_normal([num_input, hidden_units], stddev=1.0),
                     name="W1")
    B1 = tf.Variable(tf.random_normal([hidden_units], stddev=1.0), name="B1")
    A1 = tf.nn.softmax(tf.matmul(X, W1) + B1, name="A1")

    W2 = tf.Variable(tf.random_normal([hidden_units, num_class], stddev=1.0),
                     name="W2")
...
for generation in range(MAX_GEN):
    ...
    for sess in sessions:
        ...
        for _ in range(N_EPISODE):
            ...
            while True:
                ...
                # print inputs
                predicted_action, p, w1, b1, w2, b2 = sess.run(
                    [Y_index, Y_, W1, B1, W2, B2], feed_dict={X: [inputs]})
                ...
                if game.get_fitness() > 9000:
                    ...
                    if save_threshold > 100:
                        saver.save(sess, 'game_checkpoints/my_test_model',
                                   global_step=generation)
                        is_training_finished = True
                        print
                        "Saved Ultimate CheckPoint"
                        break
                if done:
    ...
    print
    "{} : {}".format(generation, fitness_data[:5])
    saver.save(sessions[0], 'game_checkpoints/my_test_model',
               global_step=generation)
    ...
    for index in range(0, POPULATION_SIZE / 4):
        ...
        if np.random.random_sample() > MUTATION_PROBABILITY:
            if np.random.random_sample() < W_MUTATION_PROBABILITY:
                ...
                # print w1_, w2
                sess.run([W1_assign, W2_assign],
                         feed_dict={W1_placeholder: w1_, W2_placeholder: w2_})
            ...
            if np.random.random_sample() < B_MUTATION_PROBABILITY:
                ...
                sess.run([B1_assign, B2_assign],
                         feed_dict={B1_placeholder: b1_, B2_placeholder: b2_})
    ...
    for index in range(0, POPULATION_SIZE / 4):
        ...
        w1_, w2_, b1_, b2_ = cross_over(w11=w11_, w12=w12_, b11=b11_, b12=b12_,
                                        w21=w21_, w22=w22_, b21=b21_, b22=b22_)
        sess.run([W1_assign, W2_assign, B1_assign, B2_assign],
                 feed_dict={W1_placeholder: w1_, W2_placeholder: w2_,
                            B1_placeholder: b1_, B2_placeholder: b2_})
        sessions.append(sess)
